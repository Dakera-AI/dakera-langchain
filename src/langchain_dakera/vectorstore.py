...
class DakeraVectorStore(VectorStore):
    ...
    def similarity_search_with_score(self, query: str, k: int = 4,
                                      filter: dict[str, Any] | None = None, **kwargs: Any) -> list[tuple[Document, float]]:
        response = self._client.query_text(self._namespace, text=query, top_k=k,
                                           filter=filter, include_text=True)
        return [(Document(page_content=r.text or "", metadata={**(r.metadata or {}), "id": r.id}), r.score)
                for r in response.results]
    ...
    def from_texts(cls, texts: list[str], embedding: Embeddings | None = None,
                   ...
                   api_url: str = "", namespace: str = "", api_key: str = "", **kwargs: Any) -> "DakeraVectorStore":
